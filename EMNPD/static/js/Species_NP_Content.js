<!-- Species_NP_Content表格分页及子表格的收缩 -->
function format_Species_NP_Content(d) {
    // `d` is the original data object for the row
    return (
        '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px; width:100%">' +
        '<tr>' +
        '<td style="width:150px">Plant Material:</td>' +
        '<td style="width:85%">' +
        d.Plant_Material +
        '</td>' +
        '</tr>' +
        ...
        '</tr>' +
        '<tr>' +
        '<td style="width:150px">Extract Activity:</td>' +
        '<td style="width:85%">' +
        d.Extract_Activity +
        '</td>' +
        '</tr>' +
...
$(document).ready(function () {
    ...
    var ajaxConfig = {url: 'http://127.0.0.1:8000/emnpd/species/' + ens_id + '/species_np_content_json/'};
    $.ajax({
        url: ajaxConfig.url,
        type: 'GET',
        dataType: 'json',
        success: function (response) {
            ...
            $.each(unitData, function (unit, unitList) {
                var tableId = 'Species_Content_' + unit.replace(' ', '_');
                ...
                var tableHTML = '<table id="' + tableId + '" class="hover row-border" style="width:100%">' +
                    ...
                    '<th>NPs ID</th>' +
                    '<th>NPs Name</th>' +
                    '<th style="text-align: center">NPs 2D Structure</th>' +
                    '<th style="width: 10%; text-align: center">Condition Index</th>' +
                ...
                $('#speciesContentTables').append(tableHTML);
                ...
                $('#' + tableId).DataTable({
                    ...
                    columns: [
                        ...
                        {data: 'ENC_ID'},
                        {data: 'NPs_Name'},
                        {data: 'NP_2D_Structure', className: 'center'},
                    ...
                    drawCallback: function (settings) {
                        ...
                        api.rows().every(function () {
                            var row = this;
                            var tr = row.node();
                            row.child(format_Species_NP_Content(row.data())).show();
                            $(tr).addClass('shown');
                        });
                    }
                });
            ...
            $('#speciesContentTables').on('click', 'tbody td.dt-control', function () {
