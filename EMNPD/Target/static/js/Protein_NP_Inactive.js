function format_Protein_NP_Inactive(d) {
...
$(document).ready(function () {
    ...
    $('#Protein_NP_Inactive').on('click', 'tbody td.dt-control', function () {
        ...
        if (row.child.isShown()) {
            ...
            // Open this row
            row.child(format_Protein_NP_Inactive(row.data())).show();
        }
    });

    $('#Protein_NP_Inactive').on('requestChild.dt', function (e, row) {
        row.child(format_Protein_NP_Inactive(row.data())).show();
    });

    var url = window.location.href;
    var ent_id = url.match(/\/protein\/(.*?)\//)[1];

    var table = $('#Protein_NP_Inactive').DataTable({
